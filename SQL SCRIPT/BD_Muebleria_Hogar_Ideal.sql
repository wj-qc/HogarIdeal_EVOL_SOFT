...
CREATE PROC SP_REGISTRARUSUARIO(
    ...
    @ApellidoMaterno varchar(50),
    @Correo varchar(100),
    @Clave varchar(100),              -- Sin el par�metro de confirmar clave
    @IdRol int,
    @Estado bit,
...
BEGIN
    ...
    -- Validar que la clave tenga una longitud m�nima
    ...
    -- Validar que el documento tenga exactamente 8 d�gitos, solo n�meros, y no tenga espacios
    IF LEN(@Documento) = 8 AND @Documento NOT LIKE '%[^0-9]%'
    BEGIN
        -- Validar que el nombre, apellido paterno y apellido materno no contengan n�meros ni caracteres especiales
        ...
        BEGIN
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    ELSE
                    BEGIN
                        SET @Mensaje = 'El correo ya est� en uso por otro usuario. Por favor, utiliza otro.'
                    END
                END
                ELSE
                BEGIN
                    SET @Mensaje = 'El n�mero de documento ya existe para otro usuario.'
                END
            END
            ELSE
            BEGIN
                SET @Mensaje = 'El correo no es v�lido. Aseg�rate de que tenga entre 5 y 20 caracteres antes del dominio y que sea de un dominio autorizado (@gmail.com o @outlook.com).'
            END
        END
        ELSE
        BEGIN
            SET @Mensaje = 'El nombre, apellido paterno y apellido materno deben tener solo letras, sin n�meros ni caracteres especiales.'
        END
    END
    ELSE
    BEGIN
        SET @Mensaje = 'El documento debe tener exactamente 8 d�gitos, solo n�meros, y no debe contener espacios.'
    END
END
GO
...
BEGIN
    ...
    IF LEN(@Documento) = 8 AND @Documento NOT LIKE '%[^0-9]%'
    BEGIN
        -- Validar que el nombre completo y apellidos no contengan n�meros ni caracteres especiales
        ...
        ELSE
        BEGIN
            SET @Mensaje = 'El nombre y apellidos deben tener solo letras, sin n�meros ni caracteres especiales.'
        END
    END
    ELSE
    BEGIN
...
CREATE PROC SP_ELIMINARUSUARIO(
    @IdUsuario int,
    @Clave varchar(100),              -- Par�metro para la clave
    @Respuesta bit OUTPUT,
    @Mensaje varchar(500) OUTPUT
)
AS
BEGIN
    ...
    -- Verificar si el usuario tiene relaci�n con una compra
    ...
    BEGIN
        SET @pasoreglas = 0
        SET @Respuesta = 0
        SET @Mensaje = @Mensaje + 'No se puede eliminar porque el usuario est� relacionado a una COMPRA.\n' 
    END

    -- Verificar si el usuario tiene relaci�n con una venta
    ...
    BEGIN
        SET @pasoreglas = 0
        SET @Respuesta = 0
        SET @Mensaje = @Mensaje + 'No se puede eliminar porque el usuario est� relacionado a una VENTA.\n' 
    END
...
BEGIN
    ...
    -- Validar que la descripci�n tenga una longitud v�lida y no contenga n�meros ni caracteres especiales
    IF LEN(@Descripcion) < 2 OR LEN(@Descripcion) > 50 OR
       PATINDEX('%[0-9]%', @Descripcion) > 0 OR 
       PATINDEX('%[^A-Za-z ]%', @Descripcion) > 0
    BEGIN
        SET @Mensaje = 'La descripci�n debe tener entre 2 y 50 caracteres y no puede contener n�meros ni caracteres especiales.'
        RETURN
    END
...
BEGIN
    ...
    BEGIN
        -- Verificar si la categor�a est� relacionada con alg�n producto
        IF NOT EXISTS (
        ...
        BEGIN
            SET @Resultado = 0
            SET @Mensaje = 'La categor�a no puede ser eliminada porque est� relacionada a un producto o proveedor.'
        END
    END
    ELSE
    BEGIN
        SET @Resultado = 0
        SET @Mensaje = 'La categor�a no existe.'
    END
END
GO
...
BEGIN
    ...
    -- Verificar si el c�digo existe para otro producto
    ...
    BEGIN
        ...
        -- Obtener el c�digo m�ximo actual
        SELECT @MaxCodigo = MAX(CAST(Codigo AS INT)) FROM PRODUCTO;

        -- Aumentar el c�digo y asignarlo al siguiente producto
...
BEGIN
 ...
 -- Validar documento (8 d�gitos num�ricos sin espacios)
    IF LEN(@Documento) = 8 AND ISNUMERIC(@Documento) = 1 AND CHARINDEX(' ', @Documento) = 0
    BEGIN
        -- Validar tel�fono (9 d�gitos, empieza con 9, solo n�meros, sin espacios)
        ...
        BEGIN
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    END
                    ELSE
                        SET @Mensaje = 'El n�mero de tel�fono ya est� en uso por otro cliente.'
                END
                ELSE
                    SET @Mensaje = 'El n�mero de documento ya existe.'
            END
            ELSE
                SET @Mensaje = 'El nombre o apellidos deben tener entre 2 y 30 caracteres, sin n�meros ni s�mbolos.'
        END
        ELSE
            SET @Mensaje = 'El tel�fono debe tener 9 d�gitos, empezar con 9 y no contener espacios.'
    END
    ELSE
        SET @Mensaje = 'El documento debe tener exactamente 8 d�gitos num�ricos sin espacios.'
END
GO
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            BEGIN
                SET @Resultado = 0
                SET @Mensaje = 'El nombre o apellidos deben tener entre 2 y 30 caracteres y no contener n�meros ni s�mbolos.'
            END
        END
        ELSE
        BEGIN
    ...
    BEGIN
        SET @Resultado = 0
        SET @Mensaje = 'El documento debe tener exactamente 8 d�gitos, solo n�meros y sin espacios.'
    END
END
GO
...
BEGIN
    ...
    BEGIN
        SET @Mensaje = 'El documento debe tener exactamente 12 d�gitos y ser num�rico.';
        RETURN;
    END

    IF LEN(@Telefono) != 9 OR @Telefono NOT LIKE '9[0-9]%'
    BEGIN
        SET @Mensaje = 'El tel�fono debe tener exactamente 9 d�gitos y comenzar con 9.';
        RETURN;
    END
    ...
    IF NOT (@RazonSocial LIKE '%[^0-9]%' AND LEN(@RazonSocial) BETWEEN 2 AND 30 
        ...
        AND @RazonSocial NOT LIKE '.%')
    BEGIN
        SET @Mensaje = 'El nombre debe tener entre 2 y 30 caracteres, no contener n�meros o m�ltiples espacios.';
        RETURN;
    END
    ...
    BEGIN
        SET @Mensaje = 'El correo debe tener entre 5 y 20 caracteres, contener al menos 5 letras, y ser de un dominio autorizado sin espacios ni s�mbolos inv�lidos.';
        RETURN;
    END

    -- Validar que no exista el documento, correo o tel�fono
    ...
    BEGIN
        SET @Mensaje = 'El n�mero de tel�fono ya est� registrado.';
        RETURN;
    END
    ...
    SET @Resultado = SCOPE_IDENTITY();
    SET @Mensaje = 'Proveedor registrado con �xito.';
END
GO
...
BEGIN
    ...
    BEGIN
        SET @Mensaje = 'El documento debe tener exactamente 8 d�gitos y ser num�rico.';
        RETURN;
    END
    ...
    SET @Mensaje = 'Proveedor modificado con �xito.';
END
GO
...
BEGIN
    ...
    BEGIN
        ...
        SET @Resultado = SCOPE_IDENTITY()
        SET @Mensaje = 'Promoción registrada exitosamente.'
    END
    ELSE
    BEGIN
        SET @Mensaje = 'Ya existe una promoción con este nombre.'
    END
END
GO
...
BEGIN
    ...
    BEGIN
        ...
        SET @Mensaje = 'Promoción editada exitosamente.'
    END
    ELSE
    BEGIN
        SET @Resultado = 0
...
BEGIN
    ...
    BEGIN
        ...
        SET @Mensaje = 'Promoción eliminada correctamente.'
    END
    ELSE
    BEGIN
        SET @Resultado = 0
        SET @Mensaje = 'La promoción no existe.'
    END
END
GO
...
INSERT INTO USUARIO(Documento, NombreCompleto, ApellidoPaterno, ApellidoMaterno, Correo, Clave, IdRol, Estado)
VALUES 
('10101010', 'Pedro', 'S�nchez', 'Castro', 'psanchez@gmail.com', '12312345', 1, 1);  

GO

INSERT INTO USUARIO(Documento, NombreCompleto, ApellidoPaterno, ApellidoMaterno, Correo, Clave, IdRol, Estado)
VALUES 
('20202020', 'Diego', 'Garc�a', 'Villanueva', 'dgarcia97@gmail.com', '45645678', 2, 1);  
...
insert into NEGOCIO(IdNegocio,Nombre,RUC,Direccion,Logo) values
  (1,'Muebler�a Hogar Ideal','20123456789','Av. Tingo Mar�a 245, Bre�a, Lima',null)
...
('23456789', 'Mar�a Alejandra', 'Garc�a', 'Marquez', 'Jr. Las Flores 456 - Lima', '923456789', 1, GETDATE()),
...
('89012345', 'Sof�a Milagros', 'Hernandez', 'Silva', 'Av. Libertad 222 - Chorrillos', '944444444', 1, GETDATE()),
('90123456', 'Luisa Maria', 'Gomez', 'Gonzales', 'Jr. San Mart�n 333 - San Borja', '922222222', 1, GETDATE()),
('01234567', 'Javier Mario', 'Vargas', 'Lloza', 'Av. Las Palmeras 444 - Los Olivos', '911111111', 1, GETDATE());


-- CREACI�N CATEGOR�A 1 --
INSERT INTO CATEGORIA (Descripcion, Estado, FechaRegistro) 
VALUES ('Sala', 1, GETDATE());

-- CREACI�N CATEGOR�A 2 --
INSERT INTO CATEGORIA (Descripcion, Estado, FechaRegistro) 
VALUES ('Comedor', 1, GETDATE());

-- CREACI�N CATEGOR�A 3 --
INSERT INTO CATEGORIA (Descripcion, Estado, FechaRegistro) 
VALUES ('Dormitorio', 1, GETDATE());

-- CREACI�N CATEGOR�A 4 --
INSERT INTO CATEGORIA (Descripcion, Estado, FechaRegistro) 
...
-- PROVEEDOR 2 - Comedor
INSERT INTO PROVEEDOR (Documento, RazonSocial, Correo, Telefono, IdCategoria, Estado, FechaRegistro)
VALUES ('20456789012', 'Dise�os Elegantes E.I.R.L.', 'disenoselegantes@outlook.com', '976543210', 2, 1, GETDATE());
...
-- PROVEEDOR 4 - Oficina
INSERT INTO PROVEEDOR (Documento, RazonSocial, Correo, Telefono, IdCategoria, Estado, FechaRegistro)
VALUES ('20678901234', 'Ofimuebles Per� E.I.R.L.', 'ofimueblesperu@outlook.com', '954321098', 4, 1, GETDATE());
